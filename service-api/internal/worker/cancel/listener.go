// Package cancel delivers per-scan cancellation signals via PostgreSQL
// LISTEN/NOTIFY, with a polling fallback if the listener can't connect.
package cancel
...
// Channel is the PostgreSQL channel used by the trigger uv_scan_cancel_notify.
const Channel = "uv_scan_cancel"
...
// Listener fans out cancellation notifications from PostgreSQL to per-scan
...
// NewListener builds a cancellation listener bound to pool.
...
func (l *Listener) Run(ctx context.Context) error {
	...
	for {
		...
		if runErr != nil && !errors.Is(runErr, context.Canceled) {
			l.logger.Warnw("Cancel listener reconnecting", zap.Error(runErr))
		}
...
func (l *Listener) runOnce(ctx context.Context) error {
	...
	l.logger.Infow("Cancel listener connected", zap.String("channel", Channel))
	...
	for {
		...
		if parseErr != nil {
			l.logger.Warnw("Can't parse cancel payload",
...
// Subscribe returns a channel that closes when scanID is cancelled, plus a
// cleanup function the caller must invoke when the scan finishes for any
// reason. Cancellation polling is run on a side-goroutine as a fallback in
// case the LISTEN connection drops.
...
func (l *Listener) poll(ctx context.Context, scanID uint64, signal chan<- struct{}) {
	...
	for {
		...
		requested, err := l.scanRepository.IsCancelRequested(ctx, scanID)
		if err != nil {
			if errors.Is(err, context.Canceled) {
				return
			}

			l.logger.Debugw("Cancel fallback poll failed",
